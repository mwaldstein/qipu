...
*None identified - all implementations match their spec requirements.*
...
- [ ] Test `workspace new --empty`
  - Implementation: `src/commands/workspace/new.rs:64`
  - Existing: `tests/cli/workspace.rs:25-46`
  - Gap: Need verification it's comprehensive
...
- [ ] Add `#[tracing::instrument]` to `IndexBuilder::build()`
  - Location: `src/lib/index/builder.rs`
  - Gap: Missing timing instrumentation

- [ ] Add `#[tracing::instrument]` to `Database::search()`
  - Location: `src/lib/db/search.rs`
  - Gap: Missing timing instrumentation

- [ ] Add instrumentation to note parse functions
  - Location: `src/lib/note/parse.rs`
  - Gap: Missing timing instrumentation
...
- [ ] Implement `--tags` and `--tier` filtering in run command
  - Parsed but ignored: `crates/llm-tool-test/src/main.rs:80-81`
...
- [x] BM25 ranking with field boosts
...
- [x] Weighted traversal: Dijkstra algorithm, edge cost formula
...
- [x] Gate types: NoteExists, LinkExists, TagExists, ContentContains, CommandSucceeds
