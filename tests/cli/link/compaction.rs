...
fn test_link_list_json_includes_compaction_annotations() {
    ...
    // link list in JSON format should include compaction annotations
    let output = qipu()
        .current_dir(dir.path())
    ...
    // JSON should contain compacts field for digest
    assert!(stdout.contains(r#""compacts": 1"#));

    // JSON should contain compaction_pct field for digest
