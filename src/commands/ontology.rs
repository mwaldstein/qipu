...
use crate::commands::format::{
    output_by_format_result, print_ontology_human, print_ontology_json, print_ontology_records,
};
...
pub fn execute_show(cli: &Cli, store: &Store) -> Result<()> {
    ...
    output_by_format_result!(cli.format,
        json => print_ontology_json(cli, store, &ontology, &note_types, &link_types),
        human => { print_ontology_human(store, &ontology, &note_types, &link_types); },
        records => { print_ontology_records(store, &ontology, &note_types, &link_types); }
    )?;
