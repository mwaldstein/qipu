...
fn output_status_human(
    ...
    note_id: &str,
    canonical: &str,
    direct_compactor: Option<&String>,
    compacted_notes: Option<&Vec<String>>,
) -> Result<()> {
...
pub fn execute(cli: &Cli, note_id: &str) -> Result<()> {
    ...
    match cli.format {
        ...
        qipu_core::format::OutputFormat::Json => {
            output_status_json(note_id, &canonical, direct_compactor, compacted_notes)?;
        }
        qipu_core::format::OutputFormat::Records => {
            output_status_records(note_id, &canonical, direct_compactor, compacted_notes);
        }
    }
