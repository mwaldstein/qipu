...
pub fn output_compaction_ids(cli: &Cli, note_id: &str, compaction_ctx: &CompactionContext) {
    ...
    if let Some((ids, truncated)) =
        compaction_ctx.get_compacted_ids(note_id, depth, cli.compaction_max_nodes)
    {
...
pub fn add_compaction_to_json(
    ...
    if let Some(obj) = json.as_object_mut() {
        ...
        add_compacted_ids_to_json(cli, note_id, obj, compaction_ctx);
    }
}
