...
use crate::commands::prime::output::{build_base_human, build_base_json, build_base_records};
...
fn estimate_base_char_count(store_path: &str, format: OutputFormat, is_empty: bool) -> usize {
    match format {
        OutputFormat::Json => build_base_json(store_path, is_empty).to_string().len(),
        OutputFormat::Human => build_base_human(store_path, is_empty).len(),
        OutputFormat::Records => build_base_records(store_path, is_empty).len(),
    }
}
