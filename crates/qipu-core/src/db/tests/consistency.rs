...
fn test_startup_validation_skips_rebuild_if_empty_db_no_notes() {
    let dir = tempdir().unwrap();
    Store::init(dir.path(), crate::store::InitOptions::default()).unwrap();
...
fn test_schema_version_outdated_rebuilds() {
    ...
    crate::db::schema::force_set_schema_version(&db.conn, 0).unwrap();
    ...
    assert_eq!(
        ...
        crate::db::schema::get_schema_version()
    );
}
...
fn test_schema_version_rollback_rebuilds() {
    ...
    crate::db::schema::force_set_schema_version(&db.conn, 999).unwrap();
