...
/// Execute `qipu compact report` (placeholder for now)
fn execute_report(_cli: &Cli, _digest_id: &str) -> Result<()> {
    // TODO: Implement report with quality metrics
    ...
    Err(crate::lib::error::QipuError::Other(
        "compact report not yet implemented".to_string(),
    ))
}
